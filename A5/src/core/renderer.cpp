...
void Renderer::render() {
    ...
    if (realTime) {
		...
		while (not_quit) {
			...
			while (SDL_PollEvent(&clk_event)) {
				// handle the quit event
				if (clk_event.type == SDL_QUIT) {
					not_quit = !not_quit;
				}
				else {
					renderpass->handleEvents(clk_event);
				}
			}
			renderpass->render();
		...
		#endif
			Sampler sampler = Sampler(260738764 + y);

			#ifdef NDEBUG // Running in release mode - Use threads
			ThreadPool::ParallelFor(0, scene.config.width, [&](int x) {
			#else // Running in debug mode - Don't use threads
			ThreadPool::SequentialFor(0, scene.config.width, [&](int x) {
			#endif
				float px, py;
				v3f sum = v3f(0, 0, 0);
				...
				for (int i = 0; i < scene.config.spp; i++) {
					...
					px = 2 * (float)px / (float)scene.config.width - 1.0;
					py = 2 * (float)py / (float)scene.config.height - 1.0;

					// shift pixels to aspect ratio and scale
					px = px * scaling * aspect_ratio;
					py = py * scaling;

					// to world coordinates
					v4f dir = v4f(px, py, -1.0, 0.0);
					dir = inverseView * dir;
					...
					v3f pixelColour = integrator->render(ray, sampler);

					sum += pixelColour;

					integrator->rgb->data[scene.config.width * (scene.config.height - y - 1) + x] = sum / scene.config.spp;
				}
			});
		});
