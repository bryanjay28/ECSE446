...
struct DirectIntegrator : Integrator {
    ...
    v3f renderMIS(const Ray& ray, Sampler& sampler) const {
		...
		if (scene.bvh->intersect(ray, info)) {
			...
			for (int i = 0; i < m_emitterSamples; i++) {
				...
				if (scene.bvh->intersect(shadowRay, sInfo)) {
					...
					if (newEmission != v3f(0.f)) {
						// get the brdf and sum up Lr
						v3f brdf = getBSDF(info)->eval(info);
						float brdf_pdf = getBSDF(info)->pdf(info);

						float ws = balanceHeuristic(m_emitterSamples, emPdf * pdf, m_bsdfSamples, brdf_pdf);

						// ensure pdf is positive
						if (pdf > 0) {
							// add up the Lr light then take hte avg later
							Lr_e += brdf * newEmission * ws / (pdf * emPdf);
						}
					}
				}
				else i--;
			...
			// iterate through based on the bsdf samples
			for (int j = 0; j < m_bsdfSamples; j++) {
				float pdf;
				v3f sample_val = getBSDF(info)->sample(info, sampler, &pdf);

				// Create our shadow ray similar to A3 with wi
				SurfaceInteraction sInfo;
				Ray shadow_ray = Ray(info.p, normalize(info.frameNs.toWorld(info.wi)));
				
				if (scene.bvh->intersect(shadow_ray, sInfo)) {
					// check where the intersection lies on the light source again
					v3f newEmission = getEmission(sInfo);
					...
					if (newEmission != v3f(0.f)) {
						...
						const Emitter& em = getEmitterByID(int(getEmitterIDByShapeID(sInfo.shapeID)));
						
						float emitterRadius = scene.getShapeRadius(em.shapeID);
						v3f emitterCenter = scene.getShapeCenter(em.shapeID);
						
						// get the emitter pdf value
						float dist = distance(emitterCenter, pShading);
						float cosThetaMax = dist / sqrt(pow(dist, 2) + pow(emitterRadius, 2));
						float pdf_emitter = Warp::squareToUniformConePdf(cosThetaMax);

						float ws = balanceHeuristic(m_bsdfSamples, pdf, m_emitterSamples, emPdf * pdf_emitter);

						Lr_b += sample_val * newEmission * ws;
					}
				}
			}
		}

		// to prevent any divisions by 0 which result in inf on the image
		if (m_emitterSamples > 0) {
			Lr += Lr_e / m_emitterSamples;
		}

		if (m_bsdfSamples > 0) {
			Lr += Lr_b / m_bsdfSamples;
		}
